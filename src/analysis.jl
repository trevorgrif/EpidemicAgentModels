...
function _analyze(model::AgentBasedModel, AgentData::DataFrame)
    AgentDataDaily = _parse_to_daily!(AgentData)
...
function _compute_spectral_radius(contact_matrix)
...
function _compute_spectral_radius_from_filename(filename, norm_days)

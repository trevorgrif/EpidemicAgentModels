...
function _get_portion_rand(model, portion, CONDITIONS = [(x) -> true], DISTRIBUTION = [1.0])
    ...
    for groupsIdx in eachindex(selectedAgentGroups)
        ...
        append!(allSelectedAgents, Distributions.sample(selectedAgentGroups[groupsIdx], sampleSizes[groupsIdx], replace = false))
    end
...
"""
function _get_portion_Watts(model, target_portion; δ = 0.014, seed_num = 1, error_radius = 0.01, delta_shift = 0.1, MAX_NEUTRAL_EFFECT = 1000)
    # Validate target_portion
    target_portion == 0.0 && return []
    target_portion > 1.0 && return []

    # Get the contact matrix of the model
    Social_Contact_Matrix = get_adjacency_matrix(model)

    # Seed an individual with the vaccine, due to random seeding, results vary remarkably...
    IC = zeros(size(Social_Contact_Matrix, 1))
    for i in Distributions.sample(1:size(Social_Contact_Matrix, 1), seed_num, replace = false)
        IC[i] = 1.0
    end
